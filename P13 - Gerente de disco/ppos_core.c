...
int mqueue_create (mqueue_t *queue, int max_msgs, int msg_size) {
    ...
    queue->buffer_size = max_msgs;
    queue->msg_size = msg_size;
    queue->first = queue->last = 0;
    queue->active = 1;

    if (sem_create(&queue->bufferSem, 1) || sem_create(&queue->sendSem, max_msgs) || sem_create(&queue->recvSem, 0))
        return -1;
...
int mqueue_send (mqueue_t *queue, void *msg) {
    ...
    memcpy(queue->buffer + (queue->last * queue->msg_size), msg, queue->msg_size);
    queue->last = (queue->last + 1) % queue->buffer_size;
...
int mqueue_recv (mqueue_t *queue, void *msg) {
    ...
    memcpy(msg, queue->buffer + (queue->first * queue->msg_size), queue->msg_size);
    queue->first = (queue->first + 1) % queue->buffer_size;
...
int mqueue_destroy (mqueue_t *queue) {
    ...
    if (sem_destroy(&queue->bufferSem) || sem_destroy(&queue->recvSem) || sem_destroy(&queue->sendSem))
        return -1;
...
/* informa o numero de mensagens presentes na fila */
